...
//  }
  ...
  test("listFrom") {
    withTempDir { tempDir =>
      ...
      assert(readStore.listFrom(deltas.head).map(_.getPath.getName).toArray ===
        Seq(1, 2, 3).map(_.toString))
      assert(readStore.listFrom(deltas(1)).map(_.getPath.getName).toArray ===
        Seq(1, 2, 3).map(_.toString))
      assert(readStore.listFrom(deltas(2)).map(_.getPath.getName).toArray ===
        Seq(2, 3).map(_.toString))
      assert(readStore.listFrom(deltas(3)).map(_.getPath.getName).toArray ===
        Seq(3).map(_.toString))
      assert(readStore.listFrom(deltas(4)).map(_.getPath.getName).toArray === Nil)
    }
  }
}
